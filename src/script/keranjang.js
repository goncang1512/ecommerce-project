...
function input(checkbox) {
  ...
  const deleteDeks = document.querySelector(".delete-deks");
...
function renderProducts(cart) {
  ...
  cart.forEach((item) => {
    ...
    <div class="w-full">
      ...
      <div
      class="keranjang-self flex w-full justify-evenly items-center h-28 text-white bg-green-600 " key=${item.id}
      >
        <p class="chekbox-container"><input type="checkbox" data-product-id="${item.id}"  class="cart-checkbox w-5 h-5 outline-0" onclick={input(this)} /></p>
        <p>
          <img
            src=${item.image}
            alt=""
